...
TEST(calculateSyndrome, correctly_calculates_error)
{
    ...
    double p = 0.1;
    double q = p;
    RhombicCode code(latticeLength, p, q, false, 1);
...
TEST(generateDataError, handles_error_probability_one)
{
    int l = 4;
    double p = 1;
    RhombicCode code(l, p, p, false, 1);
...
TEST(sweep, multiple_sweeps_per_syndrome)
{
    ...
    double p = 0.1;
    int testRate = 2;
    RhombicCode highRateCode(l, p, p, false, testRate);
    RhombicCode lowRateCode(l, p, p, false, 1);
